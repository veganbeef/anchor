...
export async function POST(request: NextRequest) {
  ...
  const { videoId, feedId } = await request.json()
  const feed = await db.selectFrom("feeds").where("id", "=", feedId).selectAll().executeTakeFirstOrThrow()

  if (Number(feed.per_video_price_usd) === 0) {
    return NextResponse.json({ error: "Video is free" }, { status: 400 })
  }
  ...
  const url = await createCheckoutSession({
    priceUsd: Number(feed.per_video_price_usd),
    mode: "payment",
    customerId,
    successUrl: `${baseUrl}/feed/${feedId}?purchased=${videoId}`,
    cancelUrl: `${baseUrl}/feed/${feedId}`,
    metadata: { feed_id: feedId, video_id: videoId, buyer_id: user.id, creator_id: feed.user_id, type: "video_purchase", product_name: `${feed.name} Video` },
